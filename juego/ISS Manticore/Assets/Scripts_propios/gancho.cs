...
public class gancho : MonoBehaviour
{
    ...
    private void Update()
    {
        if (crossHairSpinningPart != null)
        {
            ...
            if (Physics.Raycast(controlador.MainCamera.transform.position, controlador.MainCamera.transform.forward, out hitInfo, maxGrappleDistance, layerMask))
            {
                hitName = hitInfo.collider.name;
                if (Physics.Raycast(controlador.MainCamera.transform.position, controlador.MainCamera.transform.forward, out hitInfo, maxGrappleDistance))
                {
